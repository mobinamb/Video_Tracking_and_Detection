...
def joints_huber_loss(output, target, target_weight=None):
    batch_size = output.size(0)
    num_joints = output.size(1)
    heatmaps_pred = output.view((batch_size, num_joints, -1)).split(1, 1)
    heatmaps_gt = target.view((batch_size, num_joints, -1)).split(1, 1)
    ...
    for idx in range(num_joints):
        ...
        heatmap_gt = heatmaps_gt[idx]
        if target_weight is None:
            loss += 0.5 * huber_loss(heatmap_pred, heatmap_gt, reduction='mean',delta=0.003815)
        else:
            loss += 0.5 * huber_loss(
                heatmap_pred.mul(target_weight[:, idx]),
                heatmap_gt.mul(target_weight[:, idx]),
                reduction='mean',delta=0.003815
            )
    for idx in range(num_joints-1):
        heatmap_pred_next = heatmaps_pred[idx]
