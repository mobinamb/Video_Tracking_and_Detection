...
from stacked_hourglass.utils.imutils import load_image, draw_labelmap,resize,im_to_torch
...
class Generic(data.Dataset):
    ...
    def __init__(self, l, is_train=True, inp_res=(256,256), out_res=(64,64), sigma=1,
        ...
        """Initialize object."""
        self.l = l  # Image set (array of images)
    ...
    def __getitem__(self, index):
        ...
        #a1,a2,a3,a4 = list(self.dic.items())[index],list(self.dic.items())[index+1],list(self.dic.items())[index+2],list(self.dic.items())[index+3]
        #if int(a2[0].split('__')[1][:-4])-int(a1[0].split('__')[1][:-4])==1 and int(a3[0].split('__')[1][:-4])-int(a2[0].split('__')[1][:-4])==1 and int(a4[0].split('__')[1][:-4])-int(a3[0].split('__')[1][:-4])==1:
        a1,a2,a3,a4=self.l[index]
        image1,image2,image3=cv2.imread(a1[0],cv2.IMREAD_COLOR),cv2.imread(a2[0],cv2.IMREAD_COLOR),cv2.imread(a3[0],cv2.IMREAD_COLOR)
        img_paths=[a1[0],a2[0],a3[0]]
        rows, cols, colors = image1.shape
        image=np.zeros_like(image1)
        image[:,:,0]=image1[:,:,0]
        image[:,:,1]=image2[:,:,0]
        image[:,:,2]=image3[:,:,0]
        pts= torch.Tensor([[int(a1[1][0][0]*self.inp_res[0]/rows),int(a1[1][0][1]*self.inp_res[1]/cols),1],[int(a2[1][0][0]*self.inp_res[0]/rows),int(a2[1][0][1]*self.inp_res[1]/cols),1],[int(a3[1][0][0]*self.inp_res[0]/rows),int(a3[1][0][1]*self.inp_res[1]/cols),1],[int(a4[1][0][0]*self.inp_res[0]/rows),int(a4[1][0][1]*self.inp_res[1]/cols),1]])
        img = cv2.resize(image,self.inp_res,interpolation=cv2.INTER_AREA)
