...
from stacked_hourglass.loss import joints_mse_loss,joints_huber_loss
...
def do_training_step(model, optimiser, input, target, target_weight=None):
    ...
    with torch.enable_grad():
        ...
        output = model(input)
        loss = sum(joints_huber_loss(o, target, target_weight) for o in output)
...
def do_training_epoch(train_loader, model, device, optimiser,batch,sizee,sizeee):
    ...
    target = torch.empty((batch,4,sizeee,sizeee), device=device, dtype=torch.float32)
