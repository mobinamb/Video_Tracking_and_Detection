...
from stacked_hourglass.utils.evaluation import final_preds_untransformed, final_preds
...
class HumanPosePredictor:
    ...
    def prepare_image(self, image):
        ...
        if was_fixed_point:
            image /= 255.0
        if image.shape[-2:] != (1152,1152):
            image = resize(image, 1152,1152)
    ...
    def estimate_heatmaps(self, images, flip=False):
        ...
        input_tensor = torch.empty((len(raw_images), 3, 1152,1152),
    ...
    def estimate_joints(self, images, flip=False):
        ...
        coords = final_preds_untransformed(heatmaps, (288,288))
        # Rescale coords to pixel space of specified images.
        for i, image in enumerate(raw_images):
            coords[i, :, 0] *= image.shape[-1] / 288
            coords[i, :, 1] *= image.shape[-2] / 288
        if is_batched:
            return coords
        else:
            return coords[0]
...
class GenericPosePredictor:
    ...
    def estimate_joints(self, images, mean, stddev,center,scale, flip=False):
        ...
        #coords = final_preds_untransformed(heatmaps, (288,288))
        preds,coords = final_preds(heatmaps, center,scale, (288,288))
        # Rescale coords to pixel space of specified images.
        for i, image in enumerate(raw_images):
            coords[i, :, 0] *= image.shape[-1] / 288
            coords[i, :, 1] *= image.shape[-2] / 288
